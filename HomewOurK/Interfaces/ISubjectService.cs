...
namespace HomewOurK.Application.Interfaces
{
	public interface ISubjectService
	{
		...
		bool AddTeacher(int subjectId, int groupId, int teacherId);

		bool DeleteTeacher(Subject subject, Teacher teacher);

		bool DeleteTeacher(int subjectId, int groupId, int teacherId);

		IEnumerable<Teacher>? GetTeachersBySubjectId(int subjectId, int groupId);

		IEnumerable<Teacher>? GetTeachersFromSubject(Subject subject);
	}
}